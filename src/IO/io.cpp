...
Read (Mesh * mesh, std::string filename)
{
    ...
    while (!fichier.eof ())
    {
        ...
        if (mot == "Vertices")
        {
            ...
            ProgressBar progressbar ("Vertices", nbPoints);
            ...
            for (ul_t k = 0; k < nbPoints; ++k)
            {
                ...
                newPoint->SetGlobalIndex (k);

                mesh->Insert (newPoint, false);

                progressbar.PutAt (k + 1);
            }
        ...
        else if (mot == "Edges")
        {
            ...
            ProgressBar progressbar ("Edges", nbEdges);
        ...
        else if (mot == "Triangles")
        {
            ...
            ProgressBar progressbar ("Triangles", nbTriangles);
...
Write (Mesh * mesh, std::string filename)
{
    ...
    STATUS << "Points written [" << mesh->GetNumberOfPoints () << "]." << ENDLINE;
    ...
    STATUS << "Edges written [" << mesh->GetNumberOfEdges () << "]." << ENDLINE;
    ...
    STATUS << "Cells written [" << mesh->GetNumberOfCells () << "]." << ENDLINE;
